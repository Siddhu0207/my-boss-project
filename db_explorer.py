...
def get_engine(db_target: str = "dev.db") -> Engine:
    ...
    Guarantees absolute path resolution for local SQLite files.
    """
    if "://" not in db_target:
        ...
        # FIX: Ensure SQLite targets absolute project path, preventing empty CWD databases
        full_db_path = os.path.join(BASE_DIR, safe_db_name)
        return create_engine(f"sqlite:///{full_db_path}")
    ...
    try:
        ...
        for table_name in inspector.get_table_names():
            ...
            for col in inspector.get_columns(table_name):
                # FIX: Clean column types (e.g., VARCHAR(255) -> VARCHAR) to avoid breaking Mermaid syntax
                clean_type = str(col['type']).split('(')[0].replace(' ', '_')
                col_name = col['name']
                mermaid.append(f"        {clean_type} {col_name}")
            mermaid.append("    }")
        return f"### Schema Diagram (`{db_target}`)\n```mermaid\n" + "\n".join(mermaid) + "\n```"
    except Exception as e:
        return f"Error building schema diagram for `{db_target}`: {str(e)}"
...
def generate_typescript_types(db_target: str = "dev.db") -> str:
    ...
    try:
        ...
        return f"Error generating TypeScript types for `{db_target}`: {str(e)}"
...
def generate_pydantic_models(db_target: str = "dev.db") -> str:
    ...
    try:
        ...
        return f"Error generating Pydantic models for `{db_target}`: {str(e)}"
...
def explain_query_plan(query: str, db_target: str = "dev.db") -> str:
    ...
    clean_q = re.sub(r'--.*$', '', query.strip(), flags=re.MULTILINE).strip().lower()
    if not (clean_q.startswith("select") or clean_q.startswith("with")):
        return "Error: Only read-only SELECT queries are supported."
...
def run_read_query_markdown(query: str, db_target: str = "dev.db", mask_pii: bool = True) -> str:
    ...
    clean_q = re.sub(r'--.*$', '', query.strip(), flags=re.MULTILINE).strip().lower()
    if not (clean_q.startswith("select") or clean_q.startswith("with")):
        return "Error: Forbidden operation. Only read-only SELECT queries are allowed."
